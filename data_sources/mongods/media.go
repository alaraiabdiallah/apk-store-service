...
func FindAllMediaLink(filter interface{}, results *[]models.MediaLink) error {
	...
	if err := FindAllMedia(filter, &media); err != nil {
		return err
	}
	for _, res := range media {
		*results = append(*results, models.MediaLink{Url: getMediaUrl(res.Id), Version: res.Version, Flag: res.Flag, BuildCode: res.BuildCode})
	}
	return nil
}
...
func FindMedia(filter interface{}, result *models.MediaDS) error {
	...
	sort_opts.SetSort(bson.D{{"version", -1}})
	if err := coll.FindOne(dbCtx.ctx, filter, sort_opts).Decode(result); err != nil {
		return err
	}
	return nil
}
